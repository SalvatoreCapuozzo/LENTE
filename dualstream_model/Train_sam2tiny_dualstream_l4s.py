"""
train_sam2tiny_dualstream_l4s.py
=================================
...
Trainabili: TerrainEncoder + terrain_attn + SpectralEncoder + spectral_attn + mask_decoder
Frozen    : tutto il vision encoder SAM2

Bande L4S (0-13):
    0=B1  1=B2  2=B3  3=B4  4=B5  5=B6  6=B7  7=B8  8=B8A  9=B9
    10=B10  11=B11  12=DEM  13=Slope

Indici calcolati on-the-fly:
    NDVI=(B8-B4)/(B8+B4)   BSI=((B11+B4)-(B8+B2))/((B11+B4)+(B8+B2))
    NDMI=(B8-B11)/(B8+B11) NBR=(B8A-B11)/(B8A+B11) NDWI=(B3-B8)/(B3+B8)

Uso:
    python train_sam2tiny_dualstream_l4s.py
"""
...
PATIENCE          = 15
NEG_FRACTION      = 0.0
AUG_COPIES        = 2       # copie augmentate offline per ogni positivo del train
VAL_FRACTION      = 0.20
TEST_FRACTION     = 0.20
RANDOM_SEED       = 42
GRAD_ACCUM_TARGET = 8
...
class LandslideSAMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(indices[0]) > 0:
            ...
            box = torch.tensor([x_min, y_min, x_max, y_max], dtype=torch.float)
        else:
    ...
    def add_augmented_samples(self, indices: list, transform, n_copies: int, rng_seed: int = 0):
        """
        Genera n_copies versioni augmentate di ogni campione in indices
        e le aggiunge a self.data / self.positive_indices.

        Chiamato SOLO sui campioni del train set prima dell'inizio del training.
        Val e test non vengono mai toccati.

        Args:
            indices   : lista di indici originali da augmentare (solo positivi)
            transform : albumentations pipeline di augmentation
            n_copies  : quante copie per campione
            rng_seed  : seed per riproducibilita'
        """
        import random
        ...
        for orig_idx in tqdm(indices, desc=f"Augmentation offline ({n_copies}x)"):
            image_np, mask_np = self.data[orig_idx]   # HWC float32, HW float32

            for _ in range(n_copies):
                aug    = transform(image=image_np, mask=mask_np)
                aug_img  = aug["image"]   # HWC numpy (ToTensorV2 NON incluso)
                aug_mask = aug["mask"]    # HW numpy

                new_idx = len(self.data)
                self.data.append((aug_img, aug_mask))

                # Tutti i campioni augmentati sono positivi (vengono da positivi)
...
def build_train_val_test_subsets(
    ...
    """
    Split 60/20/20 con distribuzioni coerenti:

      Train : solo positivi del 60% rimanente (+ neg_fraction negativi se >0)
      Val   : solo positivi del 20% val
              -- coerente con il train, guida lo scheduler in modo pulito
      Test  : stratificato (positivi + negativi del 20% test)
              -- rappresenta la distribuzione reale del dataset

    Logica:
      1. Shuffle tutto
      2. test  = primo 20%  (stratificato, intatto)
      3. remaining = 80% restante
      4. val   = positivi del primo 20% del remaining
      5. train = positivi del rimanente 60% + neg_fraction negativi
    """
    ...
    val_indices = [i for i in val_pool if i in pos_set]   # solo positivi

    # ── Train set: positivi del pool rimanente + neg_fraction negativi ────
    ...
    print(
        ...
        f"[{len(train_indices)/total*100:.1f}% totale]  <- solo positivi"
        f"\n  Val    : {len(val_indices):5d}  ({n_pos_vl} pos + {n_neg_vl} neg)  "
        f"[{len(val_indices)/total*100:.1f}%]  <- solo positivi (coerente con train)"
        f"\n  Test   : {len(test_indices):5d}  ({n_pos_te} pos + {n_neg_te} neg)  "
        f"[{len(test_indices)/total*100:.1f}%]  <- stratificato (valutazione finale)\n"
    )
...
# ============================================================
# 3. INDICI SPETTRALI (on-the-fly, differenziabili)
# ============================================================
...
class SideStreamCrossAttention(nn.Module):
    """
    Cross-attention: query = neck features [B,C,H,W],
                     key/value = side stream [B,E,Hd,Wd]
    Output: [B,C,H,W]
    """
    ...
    def forward(self, f_rgb: torch.Tensor, z_side: torch.Tensor) -> torch.Tensor:
        ...
        if (Hd, Wd) != (H, W):
            z_side = F.interpolate(z_side, size=(H, W),
...
class SAM2TinyDualStream(nn.Module):
    """
    SAM2-Tiny con doppio stream laterale.
    Trainabili: TerrainEncoder + terrain_attn + SpectralEncoder + spectral_attn + mask_decoder
    ...
    def __init__(self, hf_path=HF_PATH, encoder_dim=ENCODER_DIM,
        ...
        self.terrain_encoder = TinyViTEncoder(
            in_channels=2, embed_dim=encoder_dim,
            num_heads=encoder_heads, num_layers=encoder_layers,
            patch_stride=patch_stride)
        self.terrain_attn = SideStreamCrossAttention(
            dim_rgb=dim_rgb, dim_side=encoder_dim,
            num_heads=SideStreamCrossAttention._safe_num_heads(dim_rgb))

        self.spectral_encoder = TinyViTEncoder(
            in_channels=5, embed_dim=encoder_dim,
            num_heads=encoder_heads, num_layers=encoder_layers,
            patch_stride=patch_stride)
        self.spectral_attn = SideStreamCrossAttention(
            dim_rgb=dim_rgb, dim_side=encoder_dim,
            num_heads=SideStreamCrossAttention._safe_num_heads(dim_rgb))

        self._z_terrain  = None
        self._z_spectral = None
    ...
    def _register_fusion_hook(self):
        ...
        if hook_mod is not None:
            ...
            for name, m in v_enc.named_modules():
                if isinstance(m, nn.Conv2d):
                    hook_mod = m; last_name = name
            if hook_mod is not None:
                print(f"  Hook: {last_name} (ultimo Conv2d)")
        if hook_mod is None:
            raise RuntimeError("Neck del vision encoder non trovato.")

        def _apply_fusion(t):
            """Applica cross-attention a un tensore [B, C, H, W]."""
            if not isinstance(t, torch.Tensor) or t.ndim != 4:
                return t
            try:
                fused = t
                if self._z_terrain  is not None:
                    fused = self.terrain_attn(fused,  self._z_terrain)
                if self._z_spectral is not None:
                    fused = self.spectral_attn(fused, self._z_spectral)
                return fused
            except Exception:
        ...
        def fusion_hook(module, args, output):
            """
            Il neck di SAM2 restituisce (fpn_features, position_encodings)
            dove fpn_features e' una lista/tupla di [B, C, H, W] a piu' scale.
            Iniettiamo la fusione SOLO sul livello a piu' bassa risoluzione
            (l'ultimo, semanticamente piu' ricco).
            """
            if self._z_terrain is None and self._z_spectral is None:
                return output

            # Caso 1: output e' un tensore singolo [B, C, H, W]
            if isinstance(output, torch.Tensor):
                return _apply_fusion(output)

            # Caso 2: tupla — di solito (features, position_encodings)
            if isinstance(output, tuple):
                features = output[0]

                # 2a: features e' un tensore singolo
                if isinstance(features, torch.Tensor) and features.ndim == 4:
                    return (_apply_fusion(features),) + output[1:]

                # 2b: features e' una lista/tupla di tensori (FPN multi-scala)
                if isinstance(features, (list, tuple)) and len(features) > 0:
                    new_features = list(features)
                    # Trova il livello a piu' bassa risoluzione (H*W minimo)
                    sizes = [
                        f.shape[-1] * f.shape[-2] if isinstance(f, torch.Tensor) and f.ndim == 4 else float("inf")
                        for f in features
                    ]
                    target_idx = sizes.index(min(sizes))
                    new_features[target_idx] = _apply_fusion(features[target_idx])
                    new_features = type(features)(new_features)
                    return (new_features,) + output[1:]
...
def criterion(y_pred: torch.Tensor, y_true: torch.Tensor, lambda_dice: float = 0.3):
    y_pred    = y_pred.contiguous()
    y_true_rs = F.interpolate(y_true.unsqueeze(1).float(),
                              size=y_pred.shape[2:], mode="nearest")
    ce   = F.binary_cross_entropy_with_logits(y_pred, y_true_rs)
...
def train():
    ckpt_path    = "best_sam2tiny_dualstream_l4s.pth"
    history_file = "history_sam2tiny_dualstream_l4s.json"
    ...
    # Augmentation offline — senza ToTensorV2 (produce numpy HWC)
    # Applicata una volta sola per generare le copie extra in RAM
    ...
    # Augmentation on-the-fly — con ToTensorV2 (usata nel DataLoader)
    # Leggera: non vogliamo double-augment pesante sulle copie sintetiche
    ...
    # ── Augmentation offline sul train set ──────────────────────────────
    # Recupera gli indici originali del train (solo positivi)
    train_pos_indices = [
        i for i in train_subset.indices
        if i in set(full_ds.positive_indices)
    ]
    full_ds.add_augmented_samples(
        indices   = train_pos_indices,
        transform = offline_aug,
        n_copies  = AUG_COPIES,
        rng_seed  = RANDOM_SEED,
    )

    # Gli indici augmentati sono stati aggiunti a full_ds.data e positive_indices.
    # Aggiungiamo i loro indici al train_subset.
    ...
    # Crea un nuovo Subset con originali + augmentati
    from torch.utils.data import Subset as _Subset
    train_subset = _Subset(full_ds, list(train_subset.indices) + aug_indices)

    print(
        f"  Train set finale: {len(train_subset)} campioni "
    ...
    print(f"\nTraining SAM2-Tiny DualStream | Epoche {start_epoch+1}/{EPOCHS} | "
    ...
    with open(history_file, "w") as f:
        json.dump(history, f, indent=4)
    print(f"Risultati test salvati in: {history_file}")
