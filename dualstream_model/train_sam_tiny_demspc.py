"""
train_sam2tiny_unifiedstream_alldata.py
=========================================
SAM2-Tiny con stream laterale unificato su Landslide4Sense.
Tutti i campioni (positivi + negativi) nel train e val.

Architettura:
  Input [B, 14, H, W]  (12 bande Sentinel-2 + DEM + Slope)
    |
    +-- Sentinel-2 [B,12,H,W] (B1-B12, ind. 0-11)
    |       patch embed espanso 3→12
    |       SAM2-Tiny vision encoder (FROZEN) --> neck
    |                                               |
    +-- SideEncoder [B,7,H,W]:                 CrossAttn
        DEM(1) + Slope(1) + NDVI + BSI +           |
        NDMI + NBR + NDWI (5 indici)          mask_decoder

Trainabili: patch_proj (12ch) + side_encoder + side_attn + mask_decoder
Frozen    : tutto il resto del vision encoder
...
class Adapter(nn.Module):
    ...
    Iniettato sui blocchi attention del vision encoder per
    stabilizzare l'addestramento con il patch embed espanso.
    """
...
class SAM2TinyDualStream(nn.Module):
    def __init__(self, hf_path=HF_PATH, encoder_dim=ENCODER_DIM,
        ...
        # Espandi patch embed 3 → 12 (tutte le bande Sentinel-2)
        self.patch_proj = self._expand_patch_embed(12)

        # Adapter sui blocchi attention (stabilizza training con patch embed espanso)
        self.adapters = self._inject_adapters(adapter_dim=64)

        dim_rgb = self._detect_vision_dim()
        print(f"  dim neck: {dim_rgb}")

        # Stream unificato: DEM(1)+Slope(1)+5 indici spettrali = 7ch
        self.side_encoder = TinyViTEncoder(7, encoder_dim, encoder_heads, encoder_layers, patch_stride)
        self.side_attn    = SideStreamCrossAttention(dim_rgb, encoder_dim, SideStreamCrossAttention._safe_num_heads(dim_rgb))
    ...
    def _inject_adapters(self, adapter_dim: int = 64) -> nn.ModuleList:
        """Inietta Adapter su tutti i blocchi attention del vision encoder tramite hook."""
        adapters = nn.ModuleList()
        v_encoder = self.backbone.vision_encoder
        for name, module in v_encoder.named_modules():
            if hasattr(module, "qkv") and isinstance(module.qkv, nn.Linear):
                ...
                module.register_forward_hook(self._make_adapter_hook(adapter))
                ...
                adapters.append(adapter)
                module.register_forward_hook(self._make_adapter_hook(adapter))
        print(f"  Adapter iniettati in {len(adapters)} blocchi attention")
        return adapters

    @staticmethod
    def _make_adapter_hook(adapter: "Adapter"):
    ...
    def _expand_patch_embed(self, in_channels: int) -> nn.Conv2d:
        """Sostituisce Conv2d(3→C) con Conv2d(in_channels→C). Copia pesi RGB, init rumore piccolo per i canali extra."""
        ...
        for name, module in v_enc.named_modules():
            if isinstance(module, nn.Conv2d) and module.in_channels == 3:
                ...
                parts  = name.split(".")
                parent = v_enc.get_submodule(".".join(parts[:-1])) if len(parts) > 1 else v_enc
    ...
    def _detect_vision_dim(self) -> int:
        ...
        try:
            ...
            v_enc.eval()
            with torch.no_grad():
                try: v_enc(torch.zeros(1, 12, 256, 256, device=dev))
    ...
    def _register_fusion_hook(self):
        ...
        def _apply_fusion(t):
            ...
            try:
                return self.side_attn(t, self._z_side) if self._z_side is not None else t
            except Exception:
                import traceback; traceback.print_exc(); return t

        def fusion_hook(module, args, output):
            if self._z_side is None: return output
    ...
    def forward(self, x: torch.Tensor, boxes: torch.Tensor):
        optical  = x[:, :12, :, :]                        # (B,12,H,W) Sentinel-2 B1-B12
        terrain  = x[:, 12:14, :, :]                      # (B, 2,H,W) DEM + Slope
        spectral = compute_spectral_indices(x)             # (B, 5,H,W) indici
        side     = torch.cat([terrain, spectral], dim=1)  # (B, 7,H,W)
        self._z_side = self.side_encoder(side)
        if boxes.ndim == 2: boxes = boxes.unsqueeze(1)
        out = self.backbone(pixel_values=optical, input_boxes=boxes, multimask_output=False)
        self._z_side = None
        scores = None
...
def train():
    ckpt_path    = "best_sam2tiny_unifiedstream_alldata.pth"
    history_file = "history_sam2tiny_unifiedstream_alldata.json"
